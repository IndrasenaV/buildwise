...
const homeUpdateSchema = Joi.object({
  ...
  requirementsList: Joi.array().items(Joi.string().allow('').trim()).optional(),
  ...
  cabinets: Joi.object().unknown(true).optional(),
  phases: Joi.array()
    .items(
      Joi.object({
        key: Joi.string()
...
async function updateAppliances(req, res) {
  const { homeId } = req.params;
  const schema = Joi.object().unknown(true).required();
  const { value, error } = schema.validate(req.body || {}, { abortEarly: false });
  if (error) {
...
async function updateCabinets(req, res) {
  const { homeId } = req.params;
  const schema = Joi.object().unknown(true).required();
  const { value, error } = schema.validate(req.body || {}, { abortEarly: false });
  if (error) {
