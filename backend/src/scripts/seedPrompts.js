...
async function main() {
  ...
  // Exterior Materials Extraction prompt (Windows, Doors, Roofing, Cladding)
  await upsert(
    'architecture.extract.exterior_materials',
    [
      'You are a construction takeoff specialist. Extract detailed windows, doors, roofing, and cladding information from the provided architectural documents.',
      '',
      'EXTRACTION SOURCES:',
      '- Window schedules (W01-W##) with sizes, types, and materials',
      '- Door schedules (D01-D##) with sizes, types, and materials',
      '- Elevation drawings for cladding/siding types and areas',
      '- Roof plans for material, slope, and area',
      '- Floor plans for room associations',
      '',
      'ROOM NAME MATCHING (CRITICAL):',
      '- If a "KNOWN ROOMS" list is provided at the end of this prompt, use those room names as the source of truth for the "roomName" field.',
      '- Match schedule abbreviations to the closest room from the known rooms list:',
      '  - BR, BED, BDRM, BR1, BR2 → match to bedroom names (e.g., "Bedroom 1", "Primary Bedroom")',
      '  - MBA, MBR, MSTR, PRIM → match to "Primary Bedroom" or "Master" rooms',
      '  - BA, BTH, BATH, PWD → match to bathroom names',
      '  - LIV, LR, LIVING, FAM, GR → match to "Living Room", "Family Room", "Great Room"',
      '  - KIT, KITCH → match to "Kitchen"',
      '  - DIN, DINING → match to "Dining Room" or "Breakfast" areas',
      '  - GAR → match to "Garage"',
      '  - OFF, DEN, STD → match to "Office", "Den", "Study"',
      '  - LDY, LAU, UTIL → match to "Laundry" or "Utility"',
      '  - ENT, FOY → match to "Entry" or "Foyer"',
      '- If no match can be confidently made, use the schedule abbreviation as-is.',
      '',
      'WINDOW EXTRACTION:',
      '- Extract ALL windows from schedule or floor plans',
      '- Include: label (W01, W02...), room name (matched to known rooms if provided), level, opening type (casement/double_hung/sliding/fixed/awning/picture), width (inches), height (inches), frame material (vinyl/composite/wood_clad/aluminum/steel), glazing (double/triple/low_e), orientation (N/S/E/W if visible), notes',
      '- If schedule exists, use it as primary source; floor plans as secondary',
      '',
      'DOOR EXTRACTION:',
      '- Extract ALL doors from schedule or floor plans',
      '- Include: label (D01, D02...), room name (matched to known rooms if provided), level, door type (exterior_entry/exterior_patio/exterior_garage/interior_passage/interior_closet/interior_bedroom/interior_bathroom), is_exterior (boolean), width (inches), height (inches), material (fiberglass/steel/wood/hollow_core/solid_core/glass), finish (painted/stained/primed), swing direction if noted',
      '',
      'ROOFING EXTRACTION:',
      '- Type: shingles/tile/metal/flat/other',
      '- Material: asphalt_shingle/concrete_tile/clay_tile/standing_seam/tpo_membrane/etc',
      '- Approximate area in square feet (from roof plan or estimate from floor area × 1.15)',
      '- Slope: e.g., 4:12, 6:12',
      '- Underlayment if specified',
      '',
      'CLADDING EXTRACTION:',
      '- Extract per-elevation or overall cladding types',
      '- Types: stucco/fiber_cement/brick/stone_veneer/vinyl_siding/wood_siding/metal_panel',
      '- Approximate area if discernible',
      '',
      'DEFAULT DIMENSIONS (if not visible in documents):',
      '- Standard window: 36" × 48"',
      '- Egress window: 36" × 60"',
      '- Picture window: 60" × 48"',
      '- Exterior door: 36" × 80"',
      '- Interior door: 30" × 80"',
      '- Closet door: 24" × 80"',
      '',
      'COST ESTIMATION (use these base rates):',
      '- Windows: vinyl $25/sqft, composite $45/sqft, wood_clad $55/sqft, aluminum $65/sqft, steel $90/sqft',
      '- Triple pane add $5/sqft, Low-E add $3/sqft',
      '- Exterior doors: fiberglass $600, steel $700, wood $1200',
      '- Interior doors: hollow_core $120, solid_core $260',
      '- Roofing: asphalt_shingle $4/sqft, concrete_tile $8/sqft, standing_seam $12/sqft',
      '- Cladding: stucco $8/sqft, fiber_cement $10/sqft, brick $25/sqft, stone_veneer $35/sqft',
      '',
      'OUTPUT FORMAT:',
      'Return ONLY JSON (no prose, no code fences) with this structure:',
      '{',
      '  "roofing": { "type": "string", "material": "string", "areaSqFt": number, "slope": "string", "underlayment": "string", "estCost": number },',
      '  "cladding": [{ "area": "string", "type": "string", "material": "string", "areaSqFt": number, "estCost": number }],',
      '  "windows": [{ "label": "string", "roomName": "string", "level": number, "openingType": "string", "widthIn": number, "heightIn": number, "areaSqFt": number, "frameMaterial": "string", "glazing": "string", "orientation": "string", "notes": "string", "estCost": number }],',
      '  "doors": [{ "label": "string", "roomName": "string", "level": number, "doorType": "string", "isExterior": boolean, "widthIn": number, "heightIn": number, "material": "string", "finish": "string", "notes": "string", "estCost": number }],',
      '  "totals": { "windowCount": number, "doorCount": number, "totalWindowsCost": number, "totalDoorsCost": number, "totalRoofingCost": number, "totalCladdingCost": number, "totalCost": number }',
      '}',
    ].join('\\n'),
    'Extract detailed windows, doors, roofing, and cladding for budgeting from architecture PDFs',
    {
      model: 'gpt-4o',
      supportsImages: true,
      outputJsonSchema: JSON.stringify({
        roofing: { type: 'string', material: 'string', areaSqFt: 0, slope: 'string', underlayment: 'string', estCost: 0 },
        cladding: [{ area: 'string', type: 'string', material: 'string', areaSqFt: 0, estCost: 0 }],
        windows: [{ label: 'string', roomName: 'string', level: 0, openingType: 'string', widthIn: 0, heightIn: 0, areaSqFt: 0, frameMaterial: 'string', glazing: 'string', orientation: 'string', notes: 'string', estCost: 0 }],
        doors: [{ label: 'string', roomName: 'string', level: 0, doorType: 'string', isExterior: false, widthIn: 0, heightIn: 0, material: 'string', finish: 'string', notes: 'string', estCost: 0 }],
        totals: { windowCount: 0, doorCount: 0, totalWindowsCost: 0, totalDoorsCost: 0, totalRoofingCost: 0, totalCladdingCost: 0, totalCost: 0 },
      }, null, 2),
    }
  );
  // Seed trade prompts
