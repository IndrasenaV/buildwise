...
export default function PlanningElectrical() {
  const { id } = useParams()
  ...
  useEffect(() => {
    api.getHome(id).then((h) => {
      setHome(h)
      const saved = h?.planning?.electrical?.notes || ''
  ...
  async function saveNotes() {
    try {
      ...
      // This is a placeholder until a dedicated endpoint is added.
      // eslint-disable-next-line no-console
      console.log('Electrical notes (local only):', notes)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Stack spacing={2}>
      ...
      <Paper variant="outlined" sx={{ p: 2 }}>
        <Typography variant="h6" gutterBottom>Rough-in plan and circuits</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          Capture lighting zones, outlet density by room, dedicated circuits (appliances, HVAC, EV), and low-voltage needs.
        </Typography>
        <TextField
          fullWidth
          multiline
          ...
          placeholder="Notes (e.g., kitchen small-appliance circuits, lighting scenes, exterior outlets, EV, data/AV)..."
        />
      </Paper>
    </Stack>
  )
