...
export default function PlanningDrywallPaint() {
  const { id } = useParams()
  ...
  useEffect(() => {
    api.getHome(id).then((h) => {
      setHome(h)
      const saved = h?.planning?.drywall_paint?.notes || ''
  ...
  async function saveNotes() {
    try {
      ...
      console.log('Drywall/Paint notes (local only):', notes)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Stack spacing={2}>
      ...
      <Paper variant="outlined" sx={{ p: 2 }}>
        <Typography variant="h6" gutterBottom>Finishes and paint schedule</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          Capture wall and ceiling finish levels, moisture areas, accent walls, trim/doors paint, and special textures.
        </Typography>
        <TextField
          fullWidth
          multiline
          ...
          placeholder="Notes (e.g., Level 5 in great room, semi-gloss baths/kitchen, ceiling color, accent walls, garage finish)..."
        />
      </Paper>
    </Stack>
  )
